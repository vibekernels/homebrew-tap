...
1. Tag and push a release in the zstd-vk repo:
   ```bash
   git tag v1.6.0
   git push origin v1.6.0
   ```

2. The **Build Homebrew Bottle** workflow in `zstd-vk` runs automatically on release publish. It builds the hybrid PGO binary, packages it as a bottle, and uploads it to the release. The CI output prints the `bottle do` block to add to the formula.
